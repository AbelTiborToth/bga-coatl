...
/**
 * BlueQuartet:
 * Scoring mechanism implementation for Prophecy Card type 22, 23 and 24
 * For Prophecy Card types check misc/card_types/prophecy.jpg
 */
class BlueQuartet implements ProphecyScorer
{
	/**
	 * color_1:
	...
	/**
	 * __construct:
	 * Constructor for new BlueQuartet object
		...
		foreach ($coatl->pieces as $piece) {
			switch ($state) {
				...
				case 1:
					...
					// if the piece doesn't have the 1st color represented on the card, go back to state 0
					else if ($piece->color !== $this->color_1) $state = 0;
					// otherwise (piece has the 1st color) stay in state 1
					break;
				case 2:
					// if the piece has the 1st color represented on the card, go to state 3
					if ($piece->color === $this->color_1) $state = 3;
				...
				case 3:
					...
					if ($piece->color === $this->color_2) return array(
						"score" => 5,
						"level" => 1
					);
					// if the piece has the 1st color represented on the card, go back to state 1
