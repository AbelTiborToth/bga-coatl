		...
		return declare("bgagame.coatl", ebg.core.gamegui, {
			...
			setup: function (gamedatas) {
				...
				for (const player_id in gamedatas.players) {
					...
					document.getElementById('counter_' + player_id + '_board').innerHTML = player.board_count;
					document.getElementById('counter_' + player_id + '_coatl').innerHTML = player.coatl_count;
				...
				this.addTooltipHtmlToClass("board_icon", `<h5>${_("Number of Pieces on board")}</h5>`);
				this.addTooltipHtmlToClass("coatl_icon", `<h5>${_("Number of finished Cóatls")}</h5>`);
				...
				dojo.query(".coatl_overall").forEach(function (node) {
					var hasHead = dojo.query(".box[type='head']", node).length > 0;
					var hasTail = dojo.query(".box[type='tail']", node).length > 0;

					if (hasHead && hasTail) {
						dojo.setAttr(node, "finished", "true");
					}
				});
			},
					...
					switch (stateName) {
						case 'discardProphecy':
							...
							if (this.stateArgs["discard_prophecy_n"] === 1) {
								this.bga.statusBar.setTitle(_('${you} must discard 1 Prophecy card from your hand'), args);
			...
			onLeavingState: function (stateName) {
				...
				if (stateName === 'assemble') {
					dojo.query(".coatl_overall").forEach(function (node) {
						...
						if (!hasHead || !hasTail) {
							dojo.setAttr(node, "finished", "false");
						}
					});
				}
			},
					...
					switch (stateName) {
						...
						case 'assemble':
							if (!args.possibleActions["undo"]) this.addActionButton('on_cancel', _('Cancel'), 'onCancel', null, false, 'gray')
								...
								if (args.possibleActions["confirmAssemble"]) {

									if (args.possibleActions["prophecy"].length === 0 && args.possibleActions["temple"].length === 0) {
										this.addActionButton('confirm_assemble_button', _('Confirm'), 'onConfirmAssemble');
									} else {
										this.addActionButton('confirm_assemble_button', _('Confirm'), 'onConfirmAssembleWithDialog');
									}
								}
								this.addActionButton('on_undo', _('Undo'), 'onUndo', null, false, 'gray')
								if (args.possibleActions["restart"]) this.addActionButton('on_restart', _('Restart'), 'onRestart', null, false, 'gray');
			...
			*/
			 ...
			 * @param evt
			 */
			onConfirmAssembleWithDialog: function (evt) {
				if (this.checkAction('confirmAssemble')) {
					this.confirmationDialog(_("Are you sure you want to confirm the Cóatl assemble and finish your turn?<br>(You have a Prophecy or Temple card you can place next to a Cóatl.)"), () => {
						this.ajaxcall("/coatl/coatl/actConfirmAssemble.html", {
							lock: true
						}, function (is_error) {
						});
			...
			notif_takePiece: function (notif) {
				...
				let n = Number(document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML) + notif.args.piece_ids.length;
				document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML = n;
			},
			...
			notif_newCoatl: function (notif) {
				...
				let n = Number(document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML) - 1;
				document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML = n;
			},
			...
			notif_score_coatl: function (notif) {
				...
				if (notif.args.activate !== undefined) {
					if (notif.args.activate) {
						if (document.querySelector(`#coatl_${notif.args.player_id}_${notif.args.coatl_id}_score_token`).getAttribute("active") !== "true") {
							document.querySelector(`#coatl_${notif.args.player_id}_${notif.args.coatl_id}_score_token`).setAttribute("active", "true");
							let n = Number(document.getElementById('counter_' + notif.args.player_id + '_coatl').innerHTML) + 1;
							document.getElementById('counter_' + notif.args.player_id + '_coatl').innerHTML = n;
						}
					} else {
						if (document.querySelector(`#coatl_${notif.args.player_id}_${notif.args.coatl_id}_score_token`).getAttribute("active") !== "false") {
			...
			notif_steal_pieces: function (notif) {
				...
				let n = Number(document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML) + 1;
				document.getElementById('counter_' + notif.args.player_id + '_board').innerHTML = n;
				let nn = Number(document.getElementById('counter_' + notif.args.opponent_id + '_board').innerHTML) - 2;
				document.getElementById('counter_' + notif.args.opponent_id + '_board').innerHTML = nn;
			},
			...
			notif_steal_piece: function (notif) {
				...
				let n = Number(document.getElementById('counter_' + notif.args.opponent_id + '_board').innerHTML) - 1;
				document.getElementById('counter_' + notif.args.opponent_id + '_board').innerHTML = n;
			},
